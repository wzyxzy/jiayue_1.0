...
public class AuthorSendActivity extends BaseActivity {
	...
	private void loadData(String title, String content) {
		RequestParams params = new RequestParams(Preferences.AUTHOR_POST_QUESTION);
		params.addQueryStringParameter("userId", UserUtil.getInstance(this).getUserId());
		params.addQueryStringParameter("bookId", getIntent().getStringExtra("bookId"));
		params.addQueryStringParameter("isAuthorReader", "1");
		params.addQueryStringParameter("title", title);
		params.addQueryStringParameter("content", content);

		DialogUtils.showMyDialog(AuthorSendActivity.this, MyPreferences.SHOW_PROGRESS_DIALOG, null, "正在发送中...", null);
		x.http().post(params, new Callback.CommonCallback<String>() {
			...
			public void onSuccess(String s) {
				...
				if (bean != null && bean.getCode().equals("SUCCESS")) {
					ActivityUtils.showToastForSuccess(AuthorSendActivity.this, "发表成功");
					Intent intent = new Intent();
					intent.setAction("android.intent.action.author");
					sendBroadcast(intent);
					finish();
				} else {
					DialogUtils.showMyDialog(AuthorSendActivity.this, MyPreferences.SHOW_ERROR_DIALOG, "获取失败", bean.getCodeInfo(), null);
				}
			}
			...
			public void onError(Throwable throwable, boolean b) {
				DialogUtils.dismissMyDialog();
				ActivityUtils.showToast(AuthorSendActivity.this, "信息发布失败，请检查网络");
			}
