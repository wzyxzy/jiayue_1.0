...
public final class QrCodeFinderView extends RelativeLayout {
	...
	public QrCodeFinderView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mLaserColor = resources.getColor(R.color.background);
	...
	private void init(Context context) {
		...
		RelativeLayout relativeLayout = (RelativeLayout) inflater.inflate(R.layout.layout_sq_scanner, this);
		FrameLayout frameLayout = (FrameLayout) relativeLayout.findViewById(R.id.qr_code_fl_scanner);
		mFrameRect = new Rect();
		LayoutParams layoutParams = (LayoutParams) frameLayout.getLayoutParams();
		layoutParams.width = ActivityUtils.getScreenWidth(context) * 3 / 4;
		layoutParams.height = ActivityUtils.getScreenWidth(context) * 3 / 4;
		mFrameRect.left = (ActivityUtils.getScreenWidth(context) - layoutParams.width) / 2;
		mFrameRect.top = ActivityUtils.getScreenHeight(context)/4;
		mFrameRect.right = mFrameRect.left + layoutParams.width;
		mFrameRect.bottom = mFrameRect.top + layoutParams.height;
	}

	public void refreshFrameRect(Context context, int mode) {
		this.removeAllViews();
		LayoutInflater inflater = LayoutInflater.from(context);
		RelativeLayout relativeLayout = (RelativeLayout) inflater.inflate(R.layout.layout_sq_scanner, this);
		FrameLayout frameLayout = (FrameLayout) relativeLayout.findViewById(R.id.qr_code_fl_scanner);
		...
		if (mode == 1) {
			...
			layoutParams.height = ActivityUtils.getScreenWidth(context) * 3 / 8;
		}
	...
	private void drawAngle(Canvas canvas, Rect rect) {
		...
		// 左上角
		canvas.drawRect(left, top, left + mAngleLength, top + mAngleLength, mPaint);
		// canvas.drawRect(left, top, left + mAngleThick, top + mAngleLength,
		// mPaint);
		// 右上角
		...
		// 右下角
		canvas.drawRect(right - mAngleLength, bottom - mAngleLength, right, bottom, mPaint);
		// canvas.drawRect(right - mAngleThick, bottom - mAngleLength, right,
		// bottom, mPaint);
	}
	...
	private void drawText(Canvas canvas, Rect rect) {
		...
		float left = (ActivityUtils.getScreenWidth(mContext) - mPaint.getTextSize() * text.length()) / 2;
...
//		canvas.drawRect(rect.left + 2, middle - 1, rect.right - 1, middle + 2, mPaint);
		...
		canvas.drawRect(rect.left + 2, slideTop - 1, rect.right - 2, slideTop + 1, mPaint);
	}
}
