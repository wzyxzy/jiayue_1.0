 ...
 *        ------------------------------------------------------------------
 */
public class ReaderCommunicationActivity extends BaseActivity implements IXListViewListener {
	...
	private void loadData() {
		...
		params.addQueryStringParameter("isAuthorReader", "0");
		...
		x.http().post(params, new Callback.CommonCallback<String>() {
			...
			public void onSuccess(String s) {
				...
				if (bean != null && bean.getCode().equals("SUCCESS")) {
					...
					DialogUtils.showMyDialog(ReaderCommunicationActivity.this, MyPreferences.SHOW_ERROR_DIALOG, "获取失败", bean.getCodeInfo(), null);
				}
			}
	...
	private void updata(CommentBean.Data comment) {
		if (comment != null) {
			...
			listview.setPullLoadEnable(false);
			listview.setPullRefreshEnable(false);
			listview.setAdapter(new CommonAdapter<CommentBean.Data.MainContents>(this, mainContents, R.layout.communication_item) {

				@Override
				public void convert(ViewHolder helper, CommentBean.Data.MainContents item, int position) {
			...
			listview.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					Intent intent = new Intent(ReaderCommunicationActivity.this, ReaderDescriptionItemActivity.class);
					String content = mainContents.get(position - 1).getContent();
					intent.putExtra("content", content);
					intent.putExtra("id", mainContents.get(position - 1).getId());
					intent.putExtra("addTime", mainContents.get(position - 1).getAddTime());
					intent.putExtra("title", mainContents.get(position - 1).getTitle());
					startActivity(intent);
				}
			});
		}
