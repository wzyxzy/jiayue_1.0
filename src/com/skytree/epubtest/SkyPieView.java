...
public class SkyPieView extends View {
	...
	private void drawPercent(Canvas canvas) {
		...
		int pixel = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, textSize, getResources()
						.getDisplayMetrics());
		paint.setTextSize(pixel);
		canvas.drawText(message, width / 2, height / 2 + (int) (height * 0.04),
				paint);
	}
	...
	public void drawPie(Canvas canvas) {
		...
		// gradient fill color
		RadialGradient gradient = new RadialGradient(getWidth() / 2,
				getHeight() / 2, rf, new int[]{brightColor, darkColor},
		...
		/*
		 * canvas.save(); //draw fill arc Path path = new Path();
		 * path.moveTo(outterBounds.centerX(), outterBounds.centerY());
		 * path.addArc(outterBounds, startAngle, endAngle);
		 * path.lineTo(outterBounds.centerX(), outterBounds.centerY()); Path
		 * innerPath = new Path(); innerPath.addArc(innerBounds, 0, 360);
		 * canvas.clipPath(path); canvas.clipPath(innerPath,
		 * Region.Op.DIFFERENCE); // 하드웨어 가속 상태에서는 오류 canvas.drawPath(path,
		 * paint); // canvas.clipPath(innerPath, Region.Op.UNION);
		 */
		oldValue = value;
		canvas.restore();
	}
