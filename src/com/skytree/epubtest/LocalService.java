...
public class LocalService extends Service {
	...
	public boolean isBookExist(String url) {
		for (int i = 0; i < app.bis.size(); i++) {
			BookInformation bi = app.bis.get(i);
			if (bi.url.equalsIgnoreCase(url))
				return true;
			if (bi.url.contains(url))
				return true;
			if (url.contains(bi.url))
				return true;
		}
		return false;
	}
	...
	public void sendProgress(int bookCode, int bytes_downloaded,
			int bytes_total, double percent) {
	...
	public void startDownload(String url, String coverUrl, String title,
			String author) {
		int bookCode = -1;
		try {
	...
	public void donwloadCover(String url, String fileName) {
		try {
			String targetFile = new String(SkySetting.getStorageDirectory()
					+ "/books/" + fileName);
			new DownloadCoverTask().execute(this.fileNameEncode(url),
					targetFile);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (c.moveToFirst()) {
				...
				if (DownloadManager.STATUS_SUCCESSFUL == c.getInt(columnIndex)) {
					String sourceString = c.getString(c
	...
	public void postDownload(int bookCode, int fileSize, String sourceFile) {
		...
		String destFolder = new String(SkySetting.getStorageDirectory()
				+ "/books");
		File df = new File(destFolder);
		df.mkdir();
		String fileName = app.sd.getFileNameByBookCode(bookCode);
		String targetFile = destFolder + "/"
				+ app.sd.getFileNameByBookCode(bookCode);
	...
	public BookInformation getBookInformation(String fileName,
			String baseDirectory, String coverPath) {
		debug(fileName);
	...
	public void deleteBookByBookCode(int bookCode) {
		...
		String coverPath = new String(SkySetting.getStorageDirectory()
				+ "/covers/" + targetName);
		coverPath.replace(".epub", ".jpg");
		app.sd.deleteRecursive(new File(targetDir));
		filePath = new String(SkySetting.getStorageDirectory() + "/downloads/"
				+ targetName);
