...
public class SkyDatabase {
	...
	class DBHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			Log.w("EPub", "Database Version : Old Version:" + oldVersion
	...
	public void deleteBookmarkByCode(int code) {
		String sql = String.format(Locale.US,
				"DELETE FROM Bookmark where Code = %d", code);
		db.execSQL(sql);
	}

	public void deleteBookByBookCode(int bookCode) {
		String sql = String.format(Locale.US,
				"DELETE FROM Book where BookCode = %d", bookCode);
		db.execSQL(sql);
	}

	public void deleteBookmarksByBookCode(int bookCode) {
		String sql = String.format(Locale.US,
				"DELETE FROM Bookmark where BookCode = %d", bookCode);
		db.execSQL(sql);
	}

	public void deleteHighlightsByBookCode(int bookCode) {
		String sql = String.format(Locale.US,
				"DELETE FROM Highlight where BookCode = %d", bookCode);
		db.execSQL(sql);
	}

	public void deletePagingsByBookCode(int bookCode) {
		String sql = String.format(Locale.US,
				"DELETE FROM Paging where BookCode = %d", bookCode);
		db.execSQL(sql);
	}
	...
	public int getBookmarkCode(PageInformation pi) {
		...
		String selectSql = String
				.format(Locale.US,
						"SELECT Code,PagePositionInChapter from Bookmark where BookCode=%d and ChapterIndex=%d",
	...
	public ArrayList<PageInformation> fetchBookmarks(int bookCode) {
		...
		String selectSql = String
				.format(Locale.US,
						"SELECT * from Bookmark where bookCode=%d ORDER BY ChapterIndex",
						bookCode);
		Cursor cursor = db.rawQuery(selectSql, null);
		while (cursor.moveToNext()) {
	...
	/*
	 * CREATE TABLE IF NOT EXISTS Paging ( 0 BookCode INTEGER NOT NULL, 1 Code
	 * INTEGER UNIQUE PRIMARY KEY AUTOINCREMENT NOT NULL, 2 ChapterIndex
	 * INTEGER, 3 NumberOfPagesInChapter INTEGER, 4 FontName TEXT, 5 FontSize
	 * INTEGER, 6 LineSpacing INTEGER, 7 Width INTEGER, 8 Height INTEGER, 9
	 * VerticalGapRatio REAL, A HorizontalGapRatio REAL, B IsPortrait INTEGER, C
	 * IsDoublePagedForLandscape INTEGER );
	 */
	public PagingInformation fetchPagingInformation(PagingInformation pgi) {
		String sql = String
				.format(Locale.US,
						"SELECT * FROM Paging WHERE BookCode=%d AND ChapterIndex=%d AND FontName='%s' AND FontSize=%d AND LineSpacing=%d AND Width=%d AND Height=%d AND HorizontalGapRatio=%f AND VerticalGapRatio=%f AND IsPortrait=%d AND IsDoublePagedForLandscape=%d",
						pgi.bookCode, pgi.chapterIndex, pgi.fontName,
						pgi.fontSize, pgi.lineSpacing, pgi.width, pgi.height,
						pgi.horizontalGapRatio, pgi.verticalGapRatio,
						pgi.isPortrait ? 1 : 0, pgi.isDoublePagedForLandscape
								? 1
								: 0);
		Cursor result = db.rawQuery(sql, null);
		if (result.moveToFirst()) {
	...
	public void updateBook(BookInformation bi) {
		ContentValues values = new ContentValues();
		if (bi.title != null && !bi.title.isEmpty())
			values.put("Title", bi.title);
		if (bi.creator != null && !bi.creator.isEmpty())
			values.put("Author", bi.creator);
		if (bi.publisher != null && !bi.publisher.isEmpty())
			values.put("Publisher", bi.publisher);
		if (bi.subject != null && !bi.subject.isEmpty())
			values.put("Subject", bi.subject);
		if (bi.type != null && !bi.type.isEmpty())
			values.put("Type", bi.type);
		if (bi.date != null && !bi.date.isEmpty())
			values.put("Date", bi.date);
		if (bi.language != null && !bi.language.isEmpty())
			values.put("Language", bi.language);
		if (bi.fileName != null && !bi.fileName.isEmpty())
			values.put("Filename", bi.fileName);
		if (bi.fileSize != -1 && bi.fileSize != 0)
			values.put("FileSize", bi.fileSize);
		if (bi.downSize != -1)
			values.put("DownSize", bi.downSize);
		values.put("IsDownloaded", (bi.isDownloaded ? 1 : 0));
		values.put("IsFixedLayout", (bi.isFixedLayout ? 1 : 0));
		values.put("IsRead", (bi.isRead ? 1 : 0));
		if (bi.url != null && !bi.url.isEmpty())
			values.put("URL", bi.url);
		if (bi.coverUrl != null && !bi.coverUrl.isEmpty())
			values.put("CoverURL", bi.coverUrl);
		if (bi.customOrder != -1)
			values.put("CustomOrder", bi.customOrder);
		if (bi.lastRead != null && !bi.lastRead.isEmpty())
			values.put("LastRead", bi.lastRead);
		values.put("IsRTL", (bi.isRTL ? 1 : 0));
		values.put("IsVerticalWriting", (bi.isVerticalWriting ? 1 : 0));
		if (bi.res0 != -1)
		...
		if (bi.res2 != -1)
			values.put("Res2", bi.res2);
		if (bi.etc != null && !bi.etc.isEmpty())
			values.put("Etc", bi.etc);
		if (bi.spread != -1)
			values.put("Spread", bi.spread);
		if (bi.orientation != -1)
			values.put("Orientation", bi.orientation);
		String where = String.format("BookCode=%d", bi.bookCode);
	...
	/*
	 * 0 BookCode INTEGER UNIQUE NOT NULL PRIMARY KEY AUTOINCREMENT, 1 Title
	 * TEXT, 2 Author TEXT, 3 Publisher TEXT, 4 Subject TEXT, 5 Type INTEGER, 6
	 * Date TEXT, 7 Language TEXT, 8 FileName TEXT, 9 Position REAL DEFAULT 0, 0
	 * IsFixedLayout INTEGER DEFAULT 0, 1 IsGlobalPagination INTEGER DEFAULT 0,
	 * 2 IsDownloaded INTEGER DEFAULT 0, 3 FileSize INTEGER DEFAULT -1, 4
	 * CustomOrder INTEGER DEFAULT 0, 5 URL TEXT, 6 CoverURL TEXT, 7 DownSize
	 * INTEGER DEFAULT -1, 8 IsRead INTEGER DEFAULT 0
	 */
	...
	public ArrayList<BookInformation> fetchBookInformations(int sortType,
			String key) {
		ArrayList<BookInformation> bis = new ArrayList<BookInformation>();
		String orderBy;
		if (sortType == 0)
		...
		if (!(key == null || key.isEmpty())) {
			condition = String.format(
					" WHERE Title like '%%%s%%' OR Author like '%%%s%%'", key,
					key);
		}
	...
	public int insertEmptyBook(String url, String coverUrl, String title,
			String author, long downloadId) {
		...
		this.insertBook(bi);
		// String lastSql =
		// "SELECT * FROM Book WHERE ID = (SELECT MAX(ID) FROM Book)";
	...
	public int insertEmptyBook(String title, String author) {
		// 1st, insert null information to make blank row in book table.
		BookInformation bi = new BookInformation();
		bi.title = title;
		bi.creator = author;
	...
	Highlights fetchHighlights(int bookCode, int chapterIndex) {
		...
		String selectSql = String
				.format(Locale.US,
						"SELECT * FROM Highlight where BookCode=%d and ChapterIndex=%d ORDER BY ChapterIndex",
	...
	Highlights fetchAllHighlights(int bookCode) {
		...
		String selectSql = String
				.format(Locale.US,
						"SELECT * FROM Highlight where BookCode=%d ORDER BY ChapterIndex",
						bookCode);
		Cursor cursor = db.rawQuery(selectSql, null);
		while (cursor.moveToNext()) {
			Highlight highlight = new Highlight();
	...
	public void deletePagingInformation(PagingInformation pgi) {
		String sql = String
				.format(Locale.US,
						"DELETE FROM Paging WHERE BookCode=%d AND ChapterIndex=%d AND FontName='%s' AND FontSize=%d AND LineSpacing=%d AND Width=%d AND Height=%d AND HorizontalGapRatio=%f AND VerticalGapRatio=%f AND IsPortrait=%d AND IsDoublePagedForLandscape=%d",
						pgi.bookCode, pgi.chapterIndex, pgi.fontName,
						pgi.fontSize, pgi.lineSpacing, pgi.width, pgi.height,
						pgi.horizontalGapRatio, pgi.verticalGapRatio,
						pgi.isPortrait ? 1 : 0, pgi.isDoublePagedForLandscape
	...
	public void insertPagingInformation(PagingInformation pgi) {
		...
		values.put("IsDoublePagedForLandscape", pgi.isDoublePagedForLandscape
				? 1
				: 0);
	...
	public void deleteHighlight(Highlight highlight) {
		String sql = String
				.format(Locale.US,
						"DELETE FROM Highlight where BookCode=%d and ChapterIndex=%d and StartIndex=%d and StartOffset=%d and EndIndex=%d and EndOffset=%d",
	...
	public void deleteHighlightByCode(int code) {
		String sql = String.format(Locale.US,
				"DELETE FROM Highlight where Code=%d", code);
		db.execSQL(sql);
		Log.w("EPub", sql);
	}
	...
	public void updateHighlight(Highlight highlight) {
		...
		String where = String
				.format(Locale.US,
						"BookCode=%d and ChapterIndex=%d and StartIndex=%d and StartOffset=%d and EndIndex=%d and EndOffset=%d",
						highlight.bookCode, highlight.chapterIndex,
						highlight.startIndex, highlight.startOffset,
						highlight.endIndex, highlight.endOffset);
